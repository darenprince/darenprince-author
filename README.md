...
> **Books page refresh:** `book.html` now ships a full six-book collection layout with larger card padding, responsive flex-grid listings (up to 3-up on desktop), click/tap cover-to-open detail modal flows, cover swap thumbnails (front/back), and a dedicated bottom “Stay Connected” CTA + mailing list block. Keep metadata + JSON-LD collection schema aligned and run `npm run generate:images` after image changes so GitHub Pages search/image manifests stay current.
>
> **Homepage featured books strip:** `index.html` now presents the “Upcoming Book” headline directly on the page background (styled kicker treatment), removes the outer wrapper shell from the featured rail container, and keeps featured cover thumbnails at their natural image ratio (no forced crop). The legacy empty purchase box above the format panel has been removed; store/format actions now live in the book preview flow below. Rebuild SCSS with `npm run styles:build` (or `npm run build:site`) so GitHub Pages serves the updated layout.
>
...
>
> **Metadata deployment gate:** `npm run lint:metadata` validates that key public pages keep required favicon, Apple touch icon, Open Graph image, Twitter image, and brand `theme-color` tags with deployable asset paths before build/deploy.
...
- `js/main.js` manages the mega menu, modernized search overlays (including quick-search chips), auth defaults, a global back-to-top arrow button, and routes header/mobile search submissions into the on-site Minisearch results page (`/pages/search.html`) with GitHub Pages asset-prefix support.
