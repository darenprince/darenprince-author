(() => {
  const MUTE_ATTENTION_DURATION_MS = 6000;
  const MUTE_BUTTON_BLUR_DELAY_MS = 200;

  const handlePlayerError = (context) => (error) => {
    console.error(`[Hero video] ${context}`, error);
  };
  ...
  const initHeroVideo = () => {
      ...
      player
        ...
        .then((muted) => {
          ...
          if (muted) {
            ...
            }, MUTE_ATTENTION_DURATION_MS);
          } else {
            muteButton.classList.remove('is-attention');
          }
        })
        .catch(handlePlayerError('reading mute state'));
    };
    ...
    player
      ...
      .catch((error) => {
        setLoading(false);
        showPlayOverlay();
        handlePlayerError('initial autoplay')(error);
      });
    ...
    player.on('ended', () => {
      ...
      hero.classList.add('is-image-active');
      showPlayOverlay();
      player.setCurrentTime(0).catch(handlePlayerError('reset after ending playback'));
    });
    ...
    if (playOverlay) {
      playOverlay.addEventListener('click', () => {
        ...
        player.play().catch((error) => {
          setLoading(false);
          showPlayOverlay();
          handlePlayerError('manual play trigger')(error);
        });
      });
    }
    ...
    if (restartBtn) {
      restartBtn.addEventListener('click', () => {
        player
          .setCurrentTime(0)
          .then(() => player.play())
          .catch(handlePlayerError('restart playback'));
      });
    }
    ...
    if (closeBtn) {
      closeBtn.addEventListener('click', () => {
        ...
        player
          ...
          .catch(handlePlayerError('close control reset'));
      });
    }

    hero.addEventListener('heroVideo:stop', () => {
      hidePauseOverlay();
      pauseReason = 'close';
      player
        .pause()
        .then(() => player.setCurrentTime(0))
        .catch(handlePlayerError('external stop reset'));
    });

    if (fullscreenBtn) {
      fullscreenBtn.addEventListener('click', () => {
        if (typeof player.requestFullscreen === 'function') {
          player.requestFullscreen().catch(handlePlayerError('fullscreen request'));
        }
      });
    }
    ...
    if (airplayBtn) {
      airplayBtn.addEventListener('click', () => {
        if (typeof player.requestAirplay === 'function') {
          player.requestAirplay().catch(handlePlayerError('airplay request'));
        } else if (typeof player.requestPictureInPicture === 'function') {
          player.requestPictureInPicture().catch(handlePlayerError('picture-in-picture request'));
        }
      });
    }
      ...
      muteButton.addEventListener('click', () => {
        player
          ...
          .then(() => {
            ...
            }, MUTE_BUTTON_BLUR_DELAY_MS);
          })
          .catch(handlePlayerError('unmute control'));
      });
    }
  };
