...
def test_acoustic_features_are_deterministic():
    signal=np.random.default_rng(42).normal(0,.05,16000); frames=signal[:4000].reshape(10,400); assert np.array_equal(rms(frames),rms(frames)); assert np.array_equal(fundamental_frequency(frames,16000),fundamental_frequency(frames,16000), equal_nan=True)
