...
import { Activity, AlertTriangle, BookOpen, Code2, FileAudio, LogOut, Menu, Moon, RefreshCw, Server, ShieldCheck, Sun, Terminal, Timer, Wifi, X } from 'lucide-react'
...
export default function DeveloperConsole({ session, signOut }) {
  ...
  const [theme, setTheme] = useState(() => localStorage.getItem('voxvector-theme') || 'dark')
  ...
  const setAppTheme = (next) => { setTheme(next); localStorage.setItem('voxvector-theme', next) }
  const choose = (id) => { setSection(id); setMobileOpen(false) }

  return <div className={`vv-app ${theme === 'light' ? 'vv-light' : ''}`}>
    <header className="vv-header">
      <div className="vv-header-inner">
        ...
        <div className="vv-header-actions">
          <Button variant="ghost" size="icon" onClick={() => setAppTheme(theme === 'dark' ? 'light' : 'dark')} aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}>{theme === 'dark' ? <Sun size={17} /> : <Moon size={17} />}</Button>
    ...
    <div className="vv-shell">
      ...
      <AnimatePresence>{mobileOpen && <motion.div className="vv-mobile-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMobileOpen(false)}><motion.aside className="vv-mobile-sheet" initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', stiffness: 380, damping: 35 }} onClick={e => e.stopPropagation()}><div className="vv-sheet-head"><span className="vv-eyebrow">Console</span><Button variant="ghost" size="icon" onClick={() => setMobileOpen(false)} aria-label="Close navigation"><X size={18} /></Button></div><SidebarContent section={section} choose={choose} /></motion.aside></motion.div>}</AnimatePresence>
      <main className="vv-main">
        {section === 'dashboard' && <Dashboard h={h} health={health} />}
...
function Metric({ label, value, detail }) { return <div className="vv-metric"><div className="vv-eyebrow">{label}</div><div className="vv-metric-value">{value}</div><div className="vv-muted truncate">{detail}</div></div> }
