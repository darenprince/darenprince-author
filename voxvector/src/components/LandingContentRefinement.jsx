...
const REFINEMENT_VERSION = '2026-08-24-header-logo-50-percent-v11'
...
/* Header ownership: this refinement no longer creates or resizes the React-owned logo. */
.vv-header-brand{width:180px!important;max-width:180px!important;min-width:0!important;flex:0 0 180px!important}.vv-header-icon,.vv-header-wordmark{display:none!important}
...
@media(prefers-reduced-motion:reduce){#workflow .vv-console-feature img,#workflow .vv-wave-path-a,#workflow .vv-wave-path-b,#workflow .vv-wave-spectrum i{animation:none!important}}
`
  document.head.appendChild(style)
}

export default function LandingContentRefinement(){useEffect(()=>{addStyles();const apply=()=>{refineWorkflowContent();refineFooterBrand()};apply();const timers=[80,300,700].map(delay=>window.setTimeout(apply,delay));return()=>{timers.forEach(window.clearTimeout);document.querySelector('#vv-landing-content-refinement-styles')?.remove()}},[]);return null}
