...
const badgeVariants = cva('inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs font-semibold leading-none transition-colors', {
  variants: {
    variant: {
      ...
      blue: 'border-[var(--vv-accent)]/30 bg-[var(--vv-accent)]/[0.08] text-[var(--vv-accent-bright)]',
...
export default function Badge({ className = '', variant, dot = true, children, ...props }) {
  const dotClass = variant === 'success' ? 'bg-emerald-400' : variant === 'warning' ? 'bg-amber-400' : variant === 'blue' ? 'bg-[var(--vv-accent-bright)]' : 'bg-[var(--vv-muted)]'
