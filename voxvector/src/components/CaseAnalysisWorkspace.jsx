...
import { Activity, AlertTriangle, CheckCircle2, Circle, Clock3, FileAudio, Pause, Play, RefreshCw, SkipBack, Volume2, ChevronDown, ChevronRight } from 'lucide-react'
...
const STAGE_NAMES = {
  file_upload_ingest: 'File Upload / Ingest', file_decode_normalization: 'File Decode and Normalization', provenance_integrity: 'Provenance and Integrity',
  channel_recording_assessment: 'Channel and Recording Assessment', speaker_identification_diarization: 'Speaker Identification / Diarization', speech_segmentation: 'Speech Segmentation',
  transcription_generation: 'Transcription Generation', transcript_alignment: 'Transcript Alignment', eligibility_reliability: 'Eligibility and Reliability',
  acoustic_feature_extraction: 'Acoustic Feature Extraction', prosodic_voice_quality: 'Prosodic and Voice Quality Analysis', temporal_pause_analysis: 'Temporal and Pause Analysis',
  linguistic_disfluency: 'Linguistic and Disfluency Analysis', question_answer_alignment: 'Question / Answer Alignment', within_speaker_baseline: 'Within Speaker Baseline',
  cross_method_evidence: 'Cross Method Evidence Assembly', evidence_convergence_conflict: 'Evidence Convergence and Conflict', candidate_classification: 'Candidate Classification',
...
function StageList({ run }) {
  ...
  return <div className="space-y-1">{stages.map((stage, index) => { const id = stage.id || stage.stage_id || stage.name || String(index); const expanded = open === id; const done = stage.status === 'completed'; const failed = ['failed', 'error'].includes(stage.status); const name = stage.name || STAGE_NAMES[id] || id; return <div className="overflow-hidden rounded-md border border-white/5" key={id}><button type="button" className="vv-status-row w-full text-left" onClick={() => setOpen(expanded ? null : id)} aria-expanded={expanded}><span>{failed ? <AlertTriangle size={14}/> : done ? <CheckCircle2 size={14}/> : <Circle size={14}/>}</span><span className="min-w-0"><span className="block truncate">{stage.number ? `${stage.number}. ` : ''}{name}</span><span className="block text-[10px] text-[var(--vv-muted)]">{id}</span></span><span className="ml-auto flex items-center gap-2 text-xs"><span>{stage.status || 'pending'}</span>{expanded ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}</span></button>{expanded && <div className="grid gap-3 border-t border-white/5 px-3 py-3 sm:grid-cols-2 lg:grid-cols-4"><Data label="Started" value={stage.started_at ? new Date(stage.started_at).toLocaleString() : '—'}/><Data label="Completed" value={stage.completed_at ? new Date(stage.completed_at).toLocaleString() : '—'}/><Data label="Duration" value={fmtMs(stage.duration_ms)}/><Data label="Outcome" value={stage.outcome || '—'}/>{stage.error && <div className="sm:col-span-2 lg:col-span-4"><Data label="Error" value={stage.error}/></div>}</div>}</div> })}</div>
}
...
export default function CaseAnalysisWorkspace({ caseResult, playbackUrl, file, onRefresh }) {
  ...
  const failed = stages.filter(stage => ['failed', 'error'].includes(stage.status)).length
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><div className="vv-eyebrow">ANALYSIS WORKSPACE</div><h1 className="mt-1 text-2xl font-semibold tracking-tight">{data?.title || 'Case analysis'}</h1><p className="vv-copy mt-1">Persistent case view · {data?.case_id || '—'}</p></div><Button variant="secondary" onClick={onRefresh}><RefreshCw size={14}/> Refresh case</Button></div>
    ...
    <section className="vv-panel"><div className="vv-panel-head"><h2><Activity size={16}/> Pipeline state</h2><span>{run?.run_id?.slice(0, 12) || '—'}</span></div><div className="vv-data-grid mt-4"><Data label="Status" value={run?.status}/><Data label="Pipeline" value={run?.pipeline_version}/><Data label="Request" value={run?.request_id?.slice(0, 16)}/><Data label="Stages" value={stages.length ? `${completed}/${stages.length} complete` : '—'}/></div><div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="vv-status-row"><span>Active</span><span className="ml-auto text-xs">{active ? (active.name || STAGE_NAMES[active.id] || active.id) : '—'}</span></div><div className="vv-status-row"><span>Failed</span><span className="ml-auto text-xs">{failed}</span></div><div className="vv-status-row"><span>Last outcome</span><span className="ml-auto text-xs">{stages.at(-1)?.outcome || '—'}</span></div></div><div className="mt-5"><StageList run={run}/></div></section>
