...
import { renderOperationalState } from '../lib/renderOperationalState'
...
function QACheck({ label,status,detail,url,tone }) { const resolvedTone=tone||stateTone(status); const Icon=resolvedTone==='healthy'?CheckCircle2:resolvedTone==='error'?AlertTriangle:CircleDashed; return <div className={`vv-qa-check ${resolvedTone}`}><Icon size={14}/><div className="min-w-0"><div className="vv-qa-check__top"><strong>{label}</strong><span>{status}</span></div><p>{detail}</p>{url&&<a className="vv-workflow-link" href={url} target="_blank" rel="noreferrer">Open workflow <ExternalLink size={11}/></a>}</div></div> }
const isReportedTrue = value => value === true
...
export default function DeveloperEngineeringStatus({ mode = 'toolbar', accessToken = '' }){
  ...
  const frontendQa=workflows.data?.frontendQa
  const backendQa=workflows.data?.backendQa
  const pagesDeploy=workflows.data?.deployment
  const frontendQaState=workflowEvidenceState(frontendQa,workflows.data?.frontendQaMatchesSource??null,workflows)
  ...
  const diarizationState=diarizationProvider==='not_configured'?'NOT CONFIGURED':isReportedTrue(diarization.execution_ready)?'READY · EXECUTION UNVERIFIED':cloudDiarization&&!isReportedTrue(diarization.pyannote_api_key_configured)?'API KEY NOT DETECTED':!cloudDiarization&&!localAdapter?'NOT INSTALLED':'CONFIGURED'
  ...
  const renderStatusState=renderOperationalState({
    serviceState: renderOperational.service_state || (renderService.suspended===true?'suspended':renderService.state||renderService.status),
    deployState: renderOperational.deploy_state || renderDeploy.status,
    pending: render.isPending,
    error: render.isError,
  })
  const renderServiceState=renderStatusState.serviceState
  const renderDeployState=renderStatusState.deployState
  const reportedRenderRevision=renderOperational.source_revision||renderDeploy?.commit?.id||renderDeploy?.commit?.sha||''
  const renderCommit=reportedRenderRevision==='unknown'?'':reportedRenderRevision
  const renderRevisionMatch=backendRevision&&renderCommit?backendRevision===renderCommit:null
  const frontendRevisionShort=frontendRevision?frontendRevision.slice(0,12):'NOT EXPOSED'
  const currentSha=backendRevision||'NOT EXPOSED'
  const backendVersion=runtime.version||h.pipeline||h.package_version||h.api_version||'NOT REPORTED'
  const healthObservedAt=runtime.observed_at||h.observed_at||''
  ...
  const launcherSummary=renderStatusState.live&&apiState==='HEALTHY'?`API HEALTHY · RENDER ${renderServiceState} · DEPLOY ${renderDeployState}`:`${apiState} · RENDER ${renderServiceState} · DEPLOY ${renderDeployState}`
  const hideStatus=()=>{setOpen(false);setHidden(true)}

  if(hidden)return null
  return <section className={`vv-eng-status vv-eng-status--${mode} ${renderStatusState.live?'is-service-live':'is-service-attention'} ${open?'is-open':'is-collapsed'}`} aria-label="Engineering status">
    ...
    {open&&<div id={panelId} className="vv-eng-status__body" role="region" aria-label="Full live engineering status">
      ...
      <div className="vv-eng-current"><div><span className="vv-eng-kicker">BACKEND RUNTIME REVISION</span><code>{currentSha}</code><small>{healthObservedAt?`Observed ${new Date(healthObservedAt).toLocaleString()}`:'Observation time not reported'}</small></div><div><span className="vv-eng-kicker">FRONTEND BUILD</span><strong>{appPackage.version}</strong><code>{frontendRevisionShort}</code><small>{workflowsObservedAt?`GitHub checked ${new Date(workflowsObservedAt).toLocaleString()}`:'GitHub observation pending'}</small></div><div><span className="vv-eng-kicker">BACKEND PIPELINE</span><strong>{backendVersion}</strong><small>{runtime.version_source||'Version source not reported'}</small></div><div><span className="vv-eng-kicker">RENDER DEPLOY</span><strong>{renderDeployState}</strong><code>{renderCommit?renderCommit.slice(0,12):'not reported'}</code><small>{renderRevisionMatch===true?'Matches backend runtime':renderRevisionMatch===false?'Differs from backend runtime':'Revision comparison unavailable'}</small></div></div>
      <div className="vv-eng-grid"><div className="vv-eng-panel"><div className="vv-eng-panel__head"><span>LIVE QA + PIPELINE CHECKS</span><span>{qaChecks.filter(item=>(item[4]||stateTone(item[1]))==='healthy').length} HEALTHY CHECKS</span></div><div className="vv-qa-list">{qaChecks.map(([label,status,detail,url,tone])=><QACheck key={label} label={label} status={status} detail={detail} url={url} tone={tone}/>)}</div></div><div className="vv-eng-panel"><div className="vv-eng-panel__head"><span>SOURCE TRACEABILITY</span><span>GITHUB</span></div><div className="vv-trace-list">{Object.values(TRACE).map(item=><CopyTrace key={item.path} item={item}/>)}</div><div className="vv-trace-note">SOURCE → COMMIT → WORKFLOW → TEST RESULT → ARTIFACT / DEPLOYMENT → RUNTIME → CONSOLE</div></div></div>
    </div>}
  </section>
}