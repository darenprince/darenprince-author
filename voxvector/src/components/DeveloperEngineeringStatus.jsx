...
import { Activity, CheckCircle2, ChevronDown, Clipboard, ExternalLink, Hammer, ShieldCheck, TestTube2, AlertTriangle, CircleDashed, RefreshCw } from 'lucide-react'
...
export default function DeveloperEngineeringStatus(){
  ...
  return <section className={`vv-eng-status ${open?'is-open':'is-collapsed'}`} aria-label="Engineering status">
    ...
    {open&&<div className="vv-eng-status__body"><div className="vv-eng-status__expanded-title"><span className="vv-eng-status__heading"><Activity size={16}/><span>LIVE ENGINEERING STATUS</span></span><span className="vv-eng-status__summary"><span>{built} BUILT</span><span>{tested}</span></span></div>
      ...
      <div className="vv-eng-current"><div><span className="vv-eng-kicker">CURRENT SOURCE REVISION</span><code>{currentSha}</code></div><div><span className="vv-eng-kicker">LAST GITHUB SYNC</span><strong>{workflows.data?.fetchedAt?new Date(workflows.data.fetchedAt).toLocaleTimeString():'—'}</strong></div><div><span className="vv-eng-kicker">AUTO REFRESH</span><strong>30 SECONDS</strong></div></div>
      <div className="vv-eng-grid"><div className="vv-eng-panel"><div className="vv-eng-panel__head"><span>LIVE QA + PIPELINE CHECKS</span><span>{qaChecks.filter(item=>item[1]==='PASS').length} PASSES</span></div><div className="vv-qa-list">{qaChecks.map(([label,status,detail,url])=><QACheck key={label} label={label} status={status} detail={detail} url={url}/>)}</div></div><div className="vv-eng-panel"><div className="vv-eng-panel__head"><span>SOURCE TRACEABILITY</span><span>GITHUB</span></div><div className="vv-trace-list">{Object.values(TRACE).map(item=><CopyTrace key={item.path} item={item}/>)}</div><div className="vv-trace-note">SOURCE → COMMIT → WORKFLOW → TEST RESULT → ARTIFACT / DEPLOYMENT → RUNTIME → CONSOLE</div></div></div>
    </div>}
  </section>
}
