...
const nextDependency = 'REAL PER-STAGE TELEMETRY AND LIFECYCLE REPORTING'

function stateTone(state) {
  if (state === 'HEALTHY' || state === 'PASS' || state === 'FUNCTIONAL') return 'healthy'
  if (state === 'BUILT' || state === 'PARTIAL' || state === 'PENDING' || state === 'UNVERIFIED' || state === 'CONDITIONAL' || state === 'QUEUED') return 'warning'
  return 'error'
}
...
function QACheck({ label, status, detail }) {
  ...
  const Icon = status === 'PASS' || status === 'FUNCTIONAL' ? CheckCircle2 : status === 'FAIL' ? AlertTriangle : CircleDashed
...
export default function DeveloperEngineeringStatus() {
  ...
  return <section className="vv-eng-status" aria-label="Engineering status">
    <button type="button" className="vv-eng-status__header" onClick={() => setOpen(value => !value)} aria-expanded={open}>
      <span className="vv-eng-status__heading"><Activity size={16}/><span>ENGINEERING STATUS</span></span>
