...
function stateTone(state) {
  ...
  if (['BUILT','PARTIAL','PENDING','UNVERIFIED','CONDITIONAL','QUEUED','NOT VALIDATED','IN_PROGRESS','STALE','UNAVAILABLE','NOT REPORTED','NOT CONFIGURED','NOT INSTALLED','INSTALLED · EXECUTION UNVERIFIED','512 MB CONSTRAINED','CONFIGURED'].includes(state)) return 'warning'
  return 'error'
}
...
export default function DeveloperEngineeringStatus({ mode = 'toolbar' }){
  ...
  const qaChecks=[
    ...
    ['AWS API','CONFIGURED','HTTPS endpoint: https://awsapi.crownlabs.tech · AWS ALB → ECS Fargate · target health verified separately in the cloud runtime audit.'],
  ]
