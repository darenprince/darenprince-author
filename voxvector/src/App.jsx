  ...
  return (
    <a href="/voxvector/" className="group flex items-center gap-3 no-underline" aria-label="VoxVector home">
      <span className="relative grid h-9 w-9 place-items-center overflow-hidden rounded-lg border border-white/[0.07] bg-white/[0.025]">
        <AudioLines size={16} className="absolute left-1.5 text-cyan-300/70" />
      ...
      <span>
        ...
        <span className="block text-[8px] font-medium uppercase tracking-[0.22em] text-white/30">Vocal intelligence</span>
      </span>
    </a>
  )
}
...
function SectionLabel({ children, icon: Icon }) {
  return <div className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-300/65">{Icon && <Icon size={13} />}{children}</div>
}
  ...
  return (
    <Reveal>
      <div className="relative">
        ...
        <TremorCard className="relative !rounded-xl !border-white/[0.06] !bg-[#0b1018] !p-0 !shadow-2xl !shadow-black/30">
          <div className="flex items-center justify-between border-b border-white/[0.05] px-5 py-4">
            <div className="flex items-center gap-3">
              <span className="grid h-8 w-8 place-items-center rounded-md bg-blue-500/10 text-blue-300"><FileAudio size={15} /></span>
              <div><div className="text-xs font-medium text-white/85">Recording analysis</div><div className="mt-0.5 text-[10px] text-white/35">Interface illustration</div></div>
            </div>
            <Badge variant="success">Eligible</Badge>
          </div>
          <div className="p-5">
            <div className="flex items-center justify-between text-[10px] text-white/35"><span>interview_sample.wav</span><span>48 kHz · mono</span></div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {[['Pitch', '246 Hz'], ['Intensity', '−18 dB'], ['Speech', '68%'], ['Pauses', '2.7 s']].map(([label, value]) => <div key={label} className="rounded-md bg-white/[0.025] px-3 py-3"><div className="text-[10px] text-white/35">{label}</div><div className="mt-1 text-sm font-semibold text-white/90">{value}</div></div>)}
            </div>
            <div className="mt-3 grid gap-3 md:grid-cols-[1fr_170px]">
              <div className="rounded-md bg-white/[0.02] p-4"><div className="flex items-center justify-between"><span className="text-xs font-medium text-white/75">Evidence activity</span><span className="text-[10px] text-white/30">Current segment</span></div><div className="mt-4 space-y-3">{[['Speech activity', 76], ['Acoustic observations', 64], ['Temporal observations', 51]].map(([label, value]) => <div key={label}><div className="mb-1 flex justify-between text-[10px] text-white/35"><span>{label}</span><span>{value}%</span></div><ProgressBar value={value} className="!bg-white/[0.06]" /></div>)}</div></div>
              <div className="rounded-md bg-white/[0.02] p-4"><div className="text-[10px] uppercase tracking-[0.15em] text-white/30">Reliability</div><div className="mt-3 text-2xl font-semibold text-white">High</div><div className="mt-1 text-[10px] leading-5 text-white/35">Quality comes before interpretation.</div></div>
            </div>
          </div>
        </TremorCard>
      </div>
    ...
    <Reveal>
      <div className="grid gap-3 lg:grid-cols-[1.45fr_0.75fr]">
        <TremorCard className="!rounded-xl !border-white/[0.06] !bg-[#0b1018] !shadow-none">
          <div className="flex items-start justify-between gap-4"><div><div className="text-xs font-medium text-white/75">Signal behavior</div><div className="mt-1 text-[10px] text-white/30">Illustrative analytical view</div></div><Badge variant="blue">Observational</Badge></div>
        ...
        <TremorCard className="!rounded-xl !border-white/[0.06] !bg-[#0b1018] !shadow-none">
          <div className="text-xs font-medium text-white/75">Evidence direction</div>
          <div className="mt-1 text-[10px] text-white/30">Convergence does not establish deception</div>
          ...
          <div className="mt-2 grid gap-2">{evidenceData.map((item, index) => <div key={item.name} className="flex items-center justify-between text-[11px] text-white/45"><span className="flex items-center gap-2"><span className={`h-1.5 w-1.5 rounded-full ${index === 0 ? 'bg-blue-400' : index === 1 ? 'bg-white/30' : 'bg-cyan-300'}`} />{item.name}</span><span className="text-white/70">{item.value}%</span></div>)}</div>
        </TremorCard>
      </div>
    </Reveal>
  )
...
function Landing() {
  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-[#070a0f] text-white antialiased">
      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.045] bg-[#070a0f]/88 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8">
          <Logo />
          <nav className="hidden items-center gap-7 text-[12px] text-white/48 md:flex" aria-label="Primary navigation">
            {[['Product', '#product'], ['How it works', '#workflow'], ['Technology', '#technology'], ['Use cases', '#use-cases'], ['Resources', '#briefing']].map(([label, href]) => <a key={label} href={href} className="transition-colors hover:text-white/90">{label}</a>)}
            <a href="/voxvector/developer" className="inline-flex items-center gap-1.5 transition-colors hover:text-white/90"><Terminal size={13} /> Developer</a>
          </nav>
      ...
      <main id="main-content">
        <section id="product" className="relative mx-auto max-w-7xl px-5 pb-20 pt-20 lg:px-8 lg:pb-28 lg:pt-28">
          ...
          <div className="grid items-center gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
            <div>
              <Reveal><div className="flex flex-wrap items-center gap-2"><Badge variant="blue">Advanced vocal deception analysis</Badge><span className="text-[10px] uppercase tracking-[0.16em] text-white/25">Evidence first</span></div></Reveal>
              <Reveal delay={0.06}><h1 className="mt-7 max-w-3xl text-[clamp(3.4rem,7.5vw,6.8rem)] font-semibold leading-[0.9] tracking-[-0.065em]">Hear the words.<br /><span className="text-white/72">Analyze the voice.</span><br /><span className="text-blue-300">Examine the evidence.</span></h1></Reveal>
              <Reveal delay={0.12}><p className="mt-7 max-w-xl text-base leading-7 text-white/50 sm:text-lg">VoxVector is an advanced vocal deception analysis engine built to investigate whether measurable patterns in spoken communication contain meaningful evidence associated with deception.</p></Reveal>
              ...
              <Reveal delay={0.24}><div className="mt-8 flex max-w-xl items-start gap-3 text-xs leading-5 text-white/35"><CircleAlert size={15} className="mt-0.5 shrink-0 text-blue-300/65" /><span>A pause, pitch change, hesitation or stress response is an observation. No individual vocal signal proves deception.</span></div></Reveal>
            </div>
            <SignalPreview />
          </div>
        </section>
        ...
        <section className="border-y border-white/[0.045] bg-white/[0.012]">
          <div className="mx-auto grid max-w-7xl grid-cols-2 divide-x divide-white/[0.045] sm:grid-cols-4 lg:px-8">
            {[['4', 'analytical stages'], ['6+', 'observation families'], ['1', 'evidence chain'], ['0', 'magic buttons']].map(([value, label]) => <div key={label} className="px-5 py-6 sm:px-8"><div className="text-2xl font-semibold tracking-tight text-white">{value}</div><div className="mt-1 text-[10px] uppercase tracking-[0.15em] text-white/25">{label}</div></div>)}
          </div>
        </section>

        <section id="workflow" className="mx-auto max-w-7xl px-5 py-24 lg:px-8 lg:py-32">
          <div className="grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
            <Reveal><div><SectionLabel icon={Layers3}>The analytical path</SectionLabel><h2 className="mt-4 max-w-xl text-3xl font-semibold tracking-[-0.04em] sm:text-5xl">A serious analysis starts before the interpretation.</h2><p className="mt-5 max-w-lg text-sm leading-7 text-white/42">VoxVector keeps reliability, evidence, candidate classification and final disposition separate so a visualization never becomes a scientific claim.</p></div></Reveal>
            <div className="grid gap-3 sm:grid-cols-2">{stages.map(({ number, title, icon: Icon, description }, index) => <Reveal key={number} delay={index * 0.04}><Card className="group h-full border-white/[0.055] bg-white/[0.018] transition-colors hover:border-blue-400/15 hover:bg-white/[0.028]"><CardHeader><div className="flex items-center justify-between"><span className="text-[11px] font-semibold tracking-[0.14em] text-white/25">{number}</span><span className="grid h-8 w-8 place-items-center rounded-md bg-blue-400/[0.06] text-blue-300/75"><Icon size={16} /></span></div><CardTitle className="mt-8 text-base">{title}</CardTitle><CardDescription className="mt-2">{description}</CardDescription></CardHeader><CardContent><div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.14em] text-white/25">{index === 0 ? 'Gate' : index === 3 ? 'Decision' : 'Analysis'} <ChevronRight size={12} /></div></CardContent></Card></Reveal>)}</div>
          </div>
        </section>

        <section id="technology" className="border-y border-white/[0.045] bg-[#090d13] py-24 lg:py-32">
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
            <Reveal><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><SectionLabel icon={BarChart3}>Technology</SectionLabel><h2 className="mt-4 max-w-2xl text-3xl font-semibold tracking-[-0.04em] sm:text-5xl">Measure what the system can actually support.</h2></div><p className="max-w-md text-sm leading-6 text-white/40">The current runtime is an observational foundation. Measurements become evidence inputs only after their quality and provenance are understood.</p></div></Reveal>
            <div className="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{methods.map(({ title, text, icon: Icon }, index) => <Reveal key={title} delay={index * 0.03}><Card className="h-full border-white/[0.055] bg-white/[0.018] transition-transform hover:-translate-y-0.5 hover:bg-white/[0.025]"><CardHeader><span className="grid h-9 w-9 place-items-center rounded-md bg-blue-400/[0.06] text-blue-300/75"><Icon size={17} /></span><CardTitle className="mt-6">{title}</CardTitle><CardDescription className="mt-2">{text}</CardDescription></CardHeader></Card></Reveal>)}</div>
            <Reveal><div className="mt-5"><Card className="border-white/[0.055] bg-white/[0.018]"><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><Network size={17} className="mt-0.5 text-cyan-300/65" /><div><div className="text-sm font-medium text-white/80">Evidence convergence and conflict</div><div className="mt-1 text-xs leading-5 text-white/35">Multiple observations can be examined together while individual signals remain evidence only.</div></div></div><div className="flex items-center gap-3 text-[10px] text-white/30"><span className="h-1.5 w-16 rounded-full bg-blue-400/75" /><span className="h-1.5 w-8 rounded-full bg-cyan-300/45" /><span>Convergence · conflict</span></div></CardContent></Card></div></Reveal>
          </div>
        </section>

        <section className="mx-auto max-w-7xl px-5 py-24 lg:px-8 lg:py-32">
          <Reveal><div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><SectionLabel icon={BarChart3}>Analytical interface</SectionLabel><h2 className="mt-4 max-w-2xl text-3xl font-semibold tracking-[-0.04em] sm:text-5xl">See the signal without hiding the uncertainty.</h2></div><p className="max-w-md text-sm leading-6 text-white/38">The interface is designed around measurable observations, evidence direction and reliability. The example below is illustrative.</p></div></Reveal>
          <EvidenceDashboard />
        </section>
      ...
      <footer className="border-t border-white/[0.045] bg-[#06090d]">
        <div className="mx-auto max-w-7xl px-5 py-14 lg:px-8">
          <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1fr]">
            <div><Logo /><p className="mt-5 max-w-sm text-xs leading-6 text-white/30">Advanced vocal deception analysis built around measurable evidence, transparent methodology and disciplined interpretation.</p><div className="mt-5 flex items-center gap-2 text-[10px] text-white/25"><LockKeyhole size={13} /> Privacy conscious analysis architecture</div></div>
          ...
          <div className="mt-12 flex flex-col gap-4 border-t border-white/[0.045] pt-6 text-[10px] text-white/22 sm:flex-row sm:items-center sm:justify-between"><span>© 2026 Crown Labs. VoxVector. All rights reserved.</span><span className="flex items-center gap-4"><a href="/voxvector/developer" className="inline-flex items-center gap-1.5 no-underline hover:text-white/55"><Code2 size={12} /> Developer</a><a href="https://github.com/darenprince/darenprince-author" className="inline-flex items-center gap-1.5 no-underline hover:text-white/55"><Github size={12} /> Source</a></span></div>
        </div>
      </footer>
    </div>
  )
...
function FooterColumn({ title, links }) {
  return <div><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/38">{title}</div><div className="mt-4 grid gap-3">{links.map(([label, href]) => <a key={label} href={href} className="text-xs text-white/30 no-underline transition-colors hover:text-white/70">{label}</a>)}</div></div>
}
