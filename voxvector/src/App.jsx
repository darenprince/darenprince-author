  ...
  return (
    <a href="/voxvector/" className="flex items-center gap-3 no-underline" aria-label="VoxVector home">
      <span className="relative grid h-9 w-9 place-items-center overflow-hidden rounded-lg border border-white/15 bg-white/[.035]">
        <span className="absolute inset-x-1.5 top-1/2 h-px bg-gradient-to-r from-blue-400 via-white to-purple-400" />
...
function Metric({ label, value, detail, accent = 'blue' }) {
  return (
    <div className="min-w-0 rounded-lg border border-white/8 bg-white/[.025] p-4">
      <div className="flex items-center justify-between gap-3 text-[11px] text-white/40"><span>{label}</span><span className={accent === 'purple' ? 'text-purple-300/70' : accent === 'green' ? 'text-emerald-300/70' : 'text-blue-300/70'}>●</span></div>
...
function SignalPanel() {
  return (
    <div className="relative overflow-hidden rounded-xl border border-white/10 bg-[#0a0e15] shadow-2xl shadow-black/40">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(54,137,255,.10),transparent_35%),radial-gradient(circle_at_88%_80%,rgba(155,85,255,.08),transparent_30%)]" />
      ...
      <div className="relative p-4 sm:p-5">
        <div className="rounded-lg border border-white/8 bg-black/25 p-3">
          ...
          <div className="relative h-28 overflow-hidden rounded-md bg-[#080c13]">
            ...
            <svg viewBox="0 0 900 140" preserveAspectRatio="none" className="absolute inset-0 h-full w-full">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          <Metric label="Intensity" value="−18 dB" detail="Stable variation" accent="purple" />
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_190px]">
          <div className="rounded-lg border border-white/8 bg-black/20 p-4">
            ...
            <div className="mt-4 space-y-2.5">
              {[['Speech activity', '76%', 'bg-emerald-400'], ['Acoustic observations', '64%', 'bg-blue-400'], ['Temporal observations', '51%', 'bg-purple-400']].map(([label, width, color]) => <div key={label}><div className="mb-1 flex justify-between text-[10px] text-white/35"><span>{label}</span><span>{width}</span></div><div className="h-1 rounded-full bg-white/7"><div className={`h-1 rounded-full ${color}`} style={{ width }} /></div></div>)}
            </div>
          </div>
...
function Landing() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#080b10] text-white">
      <div className="pointer-events-none fixed inset-0 z-0 bg-[radial-gradient(circle_at_78%_10%,rgba(45,112,255,.11),transparent_28%),radial-gradient(circle_at_88%_42%,rgba(144,73,236,.055),transparent_24%)]" />
      <header className="sticky top-0 z-50 border-b border-white/8 bg-[#080b10]/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <button onClick={() => setMenuOpen(v => !v)} className="rounded-md border border-white/10 p-2 text-white/65 md:hidden" aria-label="Toggle navigation">{menuOpen ? <X size={18} /> : <Menu size={18} />}</button>
        </div>
      ...
      <main className="relative z-10">
        ...
        <section className="border-y border-white/8 bg-white/[.012]">
        ...
        <section id="research" className="mx-auto max-w-7xl px-5 py-24 lg:px-8 lg:py-32">
          <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
            ...
            <div className="rounded-xl border border-white/8 bg-[#0a0e15] p-7 sm:p-9"><div className="flex items-center justify-between"><SectionLabel>Capability state</SectionLabel><Badge>Current runtime</Badge></div><div className="mt-7 space-y-4">{[['Implemented', 'Measurable observations and reliability controls', 'green'], ['Research', 'Additional acoustic, linguistic and learned methods', 'blue'], ['Validation', 'Task specific inferential evaluation', 'purple']].map(([title, description, tone]) => <div key={title} className="border-b border-white/7 pb-4 last:border-0 last:pb-0"><div className="flex items-center justify-between"><span className="text-sm font-medium">{title}</span><span className={`text-[10px] uppercase tracking-[.14em] ${tone === 'green' ? 'text-emerald-300/70' : tone === 'blue' ? 'text-blue-300/70' : 'text-purple-300/70'}`}>{tone === 'green' ? 'Active' : 'In development'}</span></div><p className="mt-1.5 text-xs leading-5 text-white/35">{description}</p></div>)}</div><div className="mt-8 flex items-start gap-3 rounded-lg border border-amber-300/10 bg-amber-300/[.025] p-4 text-xs leading-5 text-white/40"><CircleAlert size={15} className="mt-0.5 shrink-0 text-amber-300/65" />The current runtime does not establish scientifically validated deception inference. The product objective remains deception detection.</div></div>
          </div>
        </section>
      ...
      <footer className="relative z-10 border-t border-white/8 bg-[#070a0f]">
        <div className="mx-auto max-w-7xl px-5 py-14 lg:px-8">
          <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1fr]">
            ...
            <FooterColumn title="Legal and company" links={[['Privacy Policy', '#'], ['Terms of Service', '#'], ['Security', '#'], ['Contact', '#']]} />
          </div>
