...
export async function analyzeCaseSource(accessToken, caseId, sourceId, { signal } = {}) { const resolvedSourceId = await resolveCaseSourceId(accessToken, caseId, sourceId); return apiRequest(`/v1/cases/${encodeURIComponent(caseId)}/sources/${encodeURIComponent(resolvedSourceId)}/analyze`, { method:'POST', headers:authHeaders(accessToken), signal }) }
export async function stopAnalysisRun(accessToken, caseId, runId) { if (!caseId) throw new Error('An analysis case must be selected first.'); if (!runId) throw new Error('No active analysis run is available to stop.'); return apiRequest(`/v1/cases/${encodeURIComponent(caseId)}/runs/${encodeURIComponent(runId)}/cancel`, { method:'POST', headers:authHeaders(accessToken) }) }
