...
export async function analyzeCaseSource(accessToken, caseId, sourceId) { const resolvedSourceId = await resolveCaseSourceId(accessToken, caseId, sourceId); return apiRequest(`/v1/developer/render/analysis?case_id=${encodeURIComponent(caseId)}&source_id=${encodeURIComponent(resolvedSourceId)}`, { method:'POST', headers:authHeaders(accessToken) }) }
