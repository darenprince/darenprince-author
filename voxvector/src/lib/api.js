  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      xhr.addEventListener('load', () => {
        ...
        if (!response.ok) { const detail = typeof payload === 'object' ? payload?.detail : payload; const message = errorDetail(payload, `Upload failed (HTTP ${xhr.status}).`); const suffix = message.includes(`[request ${responseRequestId}]`) ? '' : ` [request ${responseRequestId}]`; reject(Object.assign(new Error(`${message}${suffix}`), result)); return }
