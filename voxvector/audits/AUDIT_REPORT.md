...
At approximately `2026-09-12T05:14:35Z`, direct `/health` returned HTTP 200 but omitted an allow-origin response for `http://127.0.0.1:5173`. The authenticated cases preflight returned HTTP 400, `Disallowed CORS origin`. Therefore a reachable cloud API was insufficient to make the local browser workspace usable.

The canonical `voxvector/vite.config.js` now routes development API requests through `/voxvector-api` on the local Vite origin, forwarding to the configured absolute API target. `voxvector/tests/vite-proxy.test.mjs` exercises a real local upstream and Vite server: unauthorized requests remain 401, authenticated binary bodies and query strings arrive intact, request IDs survive, and production build configuration retains its original target. `VoxVector/docs/DEPLOYMENT_BOUNDARY.md` documents commands, configuration, and cloud persistence boundaries. The Crown Labs product mirror needs no capability change: this is a development transport correction, with no production hosting or analytical architecture change.

Pre-commit verification: all **27 frontend tests passed** and the production build succeeded. Existing public-asset resolution notices remain. The running local proxy returned `/health` HTTP 200 and unauthenticated `/v1/cases` HTTP 401. After the owner signed in through the browser, the Developer Overview displayed API healthy / Render active / deployment live and Case Workbench loaded existing cases. Its current narrow viewport rendered the create-case and upload controls. The deployed backend still identified `1cc10f40bb6b`; this local frontend change does not deploy newer backend authorization code.

No authenticated request token, private recording, or transcript is included in this evidence. A generated synthetic speech fixture is prepared outside the repository; upload and processing remain pending explicit approval. This checkpoint proves authenticated entry and case listing, not a completed analysis, desktop/mobile acceptance, Stage 10 production proof, or scientific validation. Final committed-head test/build results and subsequent browser execution are recorded in the task handoff after this entry is committed. Next prompt: **VV-LOCAL-END-TO-END-SMOKE**, followed by the separately scoped **VV-STAGE10-CONTROLLED-PRODUCTION-PROOF** under #941.
