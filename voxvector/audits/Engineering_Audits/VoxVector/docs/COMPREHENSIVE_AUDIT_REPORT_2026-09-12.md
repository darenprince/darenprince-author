...
- **Code Repositories:** The main engineering code resides on GitHub under the `darenprince/voxvector` project (with a private mirror `darenprince-author`).  Both *frontend* and *backend/API* code are version-controlled there.  We observed  multiple living-doc files (`README.md`, architecture guides, MVP plans, etc.) and an Issue tracker that defines the release steps (#941, #970, etc.).  
