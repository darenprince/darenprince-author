...
1. Wrap the target template in the brand scope (default for Author + Game On! layouts). If a page should stay untouched, add `brand-exempt` to the `<body>` tag.
2. Drop the `.btn` class on your element (buttons, anchors, or `[role="button"]`).
3. Add an optional modifier such as `.btn--primary`, `.btn--forest`, `.btn--outline`, or a size ramp class like `.btn--sm`.
4. Override any `--btn-*` custom property inline or in a parent wrapper when you need a campaign-specific accent.

```html
<a class="btn btn--forest btn--lg" href="/game-on">Join Game On!</a>
```

The example above inherits the pill silhouette, large spacing tokens, and deep-green palette without writing any new CSS.
...
- 🔁 **Code reuse**: refactor any remaining gradient/legacy button helpers to consume the `button-base` mixin so hover, focus, and disabled states stay consistent.
...
Want to try a new palette in seconds? Add `style="--btn-bg: #hex; --btn-color: #hex"` directly to a button in the CMS, refresh the page, and, once you're happy, promote that combination into the variant maps above for reuse. This approach keeps experiments local and avoids altering global Sass until the palette is approved.
