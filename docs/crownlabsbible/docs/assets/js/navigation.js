;(function () {
  ...
  const applyTheme = (theme) => {
    ...
    document
      .querySelectorAll('[data-dark-logo]')
      .forEach(
        (img) => (img.src = theme === 'light' ? img.dataset.lightLogo : img.dataset.darkLogo)
      )
  }
  ...
  navToggle?.addEventListener('click', () => {
    if (innerWidth < 981) document.body.classList.toggle('nav-open')
      ...
      localStorage.setItem('navCollapsed', String(app.classList.contains('nav-collapsed')))
    }
  })
  ...
  if (toolbar && toolbarToggle) {
    ...
    toolbarToggle.addEventListener('click', () => {
      ...
      localStorage.setItem(
        'toolbarCollapsed',
        String(toolbar.classList.contains('toolbar-collapsed'))
      )
    })
  }
  ...
  const assets = {
    ...
    crownPsychologyArchitecture:
      '../../../../assets/images/EA6882C5-BB64-412B-9F58-395D1208F112.png',
  ...
  const path = () =>
    decodeURIComponent(new URLSearchParams(location.search).get('doc') || '').toLowerCase()
  const profile = (p) => {
    const summary = /executive[_ -]?summary|overview/.test(p)
    if (p.includes('crown-psychology'))
      return {
        alt: 'Crown Psychology',
        logo: assets.crownPsychologyLogo,
        icon: assets.crownPsychologyIcon,
        hero: p.includes('architecture')
          ? assets.crownPsychologyArchitecture
          : summary
            ? assets.crownPsychologyHero
            : '',
      }
    if (p.includes('sentinel-vault'))
      return {
        alt: 'Sentinel Vault',
        logo: assets.sentinelLogo,
        icon: assets.sentinelIcon,
        hero: summary ? assets.sentinelHero : '',
      }
    if (p.includes('crowncode-ai'))
      return {
        alt: 'CrownCode.ai',
        logo: assets.crownCodeLogo,
        icon: assets.crownCodeIcon,
        hero: summary ? assets.crownCodeHero : '',
      }
    if (p.includes('picdetective'))
      return { alt: 'PicDetective', logo: assets.picLogo, icon: assets.picIcon, hero: '' }
    if (p.includes('operation-phoenix'))
      return { alt: 'Operation Phoenix', icon: assets.phoenixIcon }
  ...
  function brandDocument() {
    ...
    if (!heading) return
    const insert = (src, cls, alt) => {
      if (!src) return
      const img = document.createElement('img')
      img.src = src
      img.className = cls
      img.alt = alt
      img.dataset.docBrand = 'true'
      heading.before(img)
    }
  ...
  brandDocument()
  new MutationObserver(brandDocument).observe(
    document.querySelector('.cl-content') || document.body,
