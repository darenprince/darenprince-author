...
| `generate:images`    | `node scripts/generate-image-manifest.js`                                                              | Catalog repo imagery (`**/*.{png,jpg,jpeg,gif,svg,webp}`; excludes node_modules/build artifacts) into `assets/image-manifest.json`. | Powers `image-index.html` and press tooling with copy-ready URLs.                                                            |
