...
- Refactored core author-platform pages (`index.html`, `home.html`, `book.html`, `meet-daren-prince.html`, `contact.html`, and `login.html`) to consume shared header/footer partials (`partials/site-header.html`, `partials/site-footer.html`) through the build-time sync script `scripts/sync-shared-shell.mjs`, reducing drift while keeping static HTML deployable for GitHub Pages.
...
- Updated npm scripts with `sync:shared-shell` and wired it into `build:site` so shared shell updates are always applied before deploy builds.
- Preserved GitHub Pages deployment compatibility by keeping static paths and metadata/social/favicons as committed artifacts.
