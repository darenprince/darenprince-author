...
4. Run `scripts/bootstrap-admin.js` after seeding credentials to elevate accounts — the utility now targets `public.profiles.user_id`, matching the latest schema.
...
5. Netlify fallback endpoint `/.netlify/functions/supabase-config` (read-only anon credentials)
...
Aliases (`SUPABASE_URL`, `SUPABASE_DB_URL`, `SUPABASE_PROJECT_URL`, `SUPABASE_REST_URL`, `SUPABASE_API_KEY`, `SUPABASE_KEY`, `SUPABASE_PUBLIC_API_KEY`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `PUBLIC_SUPABASE_URL`, `PUBLIC_SUPABASE_ANON_KEY`, `PUBLIC_SUPABASE_PUBLISHABLE_KEY`) are supported. Run `npm run build` whenever credentials change so `assets/js/env.js` stays in sync.

`/.netlify/functions/supabase-config` exposes the URL + anon key without bundling secrets. When Netlify env vars are present but the build misses `assets/js/env.js`, the browser fetches the endpoint, hydrates `localStorage.supabaseRuntimeConfig`, and reinitializes the shared client. The function now reports which aliases were detected (or missing) so Netlify logs immediately reveal misconfigurations.

Every resolution path emits structured diagnostics through `supabase/diagnostics.js`. The traces surface in:

- The Netlify function logs (server-side fallbacks, method rejections, missing env vars).
- The in-browser Supabase logger overlay (`window.supabaseDebug.show()`), including masking for sensitive keys.
- `getSupabaseDiagnostics()` for programmatic inspection in tests and scripts.

### Browser overrides for QA

- The **login page** exposes a floating “Supabase config” panel (`js/supabase-runtime-config.js`). Drop in a project URL, anon key, and optional JWT secret to refresh the client instantly—no rebuild required.
...
Deploy with `supabase functions deploy admin-users`. `npm run deploy:supabase` now runs `supabase secrets set --env-file …` ahead of every function deploy so `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, the anon key, and optional JWT secrets reach the edge runtime automatically.
...
- `supabase/diagnostics.js` — automated resolution/error logging that feeds the overlay and Netlify/server consoles. Run `window.supabaseDebug.enable({ open: true })` to view live traces of credential discovery in production.
