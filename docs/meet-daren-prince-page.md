...
2. **Origin Story Block:** Dual-headshot media stack with the updated narrative copy and a signature quote.
...
- Open Graph + Twitter image alt metadata explicitly describes Daren’s professional headshot for accessibility and richer social previews.
- JSON-LD uses `@type: Person` with `sameAs`, `award`, `authorOf`, `subjectOf`, and `hasPart` (`ImageObject`) references so search engines can surface richer author cards.
- Canonical URL: `https://darenprince.com/about`.

## Asset Sources

- Headshots: `/assets/images/daren-prince-profile-lg.jpg`, `/assets/images/2AA31BBC-1B94-4459-8B4E-E20162EDC5FD.png`.
