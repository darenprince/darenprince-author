...
- Current deploy finished: `2026-09-10T21:36:23.3655Z`
...
No fresh `/health` payload for deployed `420536...` is recorded by the current synchronization pass. The most recent previously documented fresh `/health` readback belongs to an older deployed revision and is not reused as current runtime evidence.
...
|---|---|---|
| Public React application | `https://darenprince.com/voxvector/` | Canonical GitHub Pages frontend; exact-main Pages workflow `34532394423` succeeded for `420536...`; browser verification remains separate |
| Original API | `https://voxvector.crownlabs.tech` | Preserved Render API; deploy `dep-dahi2ics728c73b6ujug` is `live` on exact `420536...`; fresh `/health` readback still separate |
...
GitHub Actions on exact `main` `420536771875c6948be51851118b58cb04a596e6` reports:
...
These are repository/build/publication workflow results. They do not establish the current Render `/health` payload, controlled provider execution on `420536...`, browser verification or scientific validation.

## Controlled production result — historical evidence from 2026-09-10

The controlled reference WAV evidence below belongs to deployed source `f0dda13694bd17ae3347e9e0eaf73e54a379fbb2`. It is intentionally preserved as historical incident/provider evidence and is not relabeled as execution of current `420536...`.
...
This is real historical provider-execution evidence. It is not evidence that current `420536...` has completed the same path, transcript truthfulness, browser verification or scientific validation.

## Merged #941 source repair and reopened production proof

PR #962 merged the first containment/durability foundation. PR #967 then merged the reviewed follow-up as current `main` `420536...`.
...
That profile has historical controlled execution evidence on `f0dda136...`. The merged current source includes the durability and Stage 10 safety repairs, but current production execution/readback is still required under reopened #941 after #964 establishes the final runtime configuration.
...
Current connected Render service and the owner-provided `2026-09-10T21:38:48Z` export build `requirements-speech.txt`, while root `render.yaml` declares `requirements-transcription.txt`. `requirements-speech.txt` includes local `pyannote.audio` in addition to faster-whisper; `requirements-transcription.txt` contains faster-whisper only.
...
Current `main` `AuthGate.jsx` still does not issue the requested login-time API wake. Draft PR #974 contains the candidate one-shot wake repair and generalizes the existing self-profile implementation across developer/admin/user roles without creating duplicate profile stores. The user workspace receives User Profile; the existing Developer Console profile route becomes role-aware for Developer Profile/Admin Profile; Admin User Management remains separately privileged.

PR #974 is not current source or deployed behavior. Its prior QA was against the pre-#967 base, so #931 requires current-main integration QA/Preview before merge recommendation plus authenticated desktop/mobile browser verification and profile save/reload afterward.

## Active issue state

- #964 — **P0 Ready / current first source-config task.** Reconcile the sole root Render Blueprint with live/exported service state.
- #941 — **P0 Reopened for production proof.** Source fixes merged/deployed; controlled rerun/readback remains after #964.
...
That persistence is not proof of current `420536...` execution or browser rehydration. #963 owns the old-case playback/transcript/artifact return path.
