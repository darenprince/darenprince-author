...
The frontend uses a product owned shadcn style component system with Base UI headless primitives, application-owned SVG for the active analytical graphics, Tailwind CSS, and Motion for React. Recharts 3 remains declared in the package manifest but is not imported by the current frontend source.
...
Application-owned SVG is used for current analytical chart and data blocks. Do not describe Recharts as an active implementation or reintroduce its runtime without an explicit, tested migration. Application-owned layout and typography remain the controlling visual layer.
...
The application-owned `NativeAreaChart` SVG component is used for secondary analytical relationships where a chart is appropriate. These graphics are explicitly labeled as illustrative and must never be presented as real subject measurements.
