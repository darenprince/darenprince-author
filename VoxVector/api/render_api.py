...
def _render_get(path: str, api_key: str, params: dict | None = None) -> dict:
    query = f"?{urlencode(params or {}, doseq=True)}" if params else ""
    request = UrlRequest(f"{RENDER_API_BASE}{path}{query}", headers={"Authorization": f"Bearer {api_key}", "Accept": "application/json"}, method="GET")
    try:
...
def render_status(service_id: str | None = Query(default=None), log_minutes: int = Query(default=30, ge=1, le=120), _: dict = Depends(require_developer)):
    api_key, resolved_service = _config(service_id)
    service = _render_get(f"/services/{resolved_service}", api_key)
    ...
    owner_id = service.get("ownerId") or service.get("owner_id") or service.get("owner", {}).get("id") if isinstance(service.get("owner"), dict) else None
    instances = _render_get(f"/services/{resolved_service}/instances", api_key, {"limit": 20})
    instance_rows = _rows(instances)
    return {
        "status": "ok",
        "service": {
...
def render_logs(service_id: str | None = Query(default=None), minutes: int = Query(default=10, ge=1, le=60), limit: int = Query(default=120, ge=1, le=100), _: dict = Depends(require_developer)):
    api_key, resolved_service = _config(service_id)
    service = _render_get(f"/services/{resolved_service}", api_key)
    owner_id = service.get("ownerId") or service.get("owner_id")
    if not owner_id and isinstance(service.get("owner"), dict):
        owner_id = service["owner"].get("id")
    if not owner_id:
    ...
    payload = _render_get("/logs", api_key, {
        "ownerId": owner_id,
    ...
    return {"status": "ok", "service_id": resolved_service, "owner_id": owner_id, "logs": _rows(payload), "observed_at": end.isoformat()}
