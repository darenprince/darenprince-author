...
app.add_middleware(CORSMiddleware, allow_origins=[origin.strip() for origin in os.getenv("CORS_ORIGINS", "*").split(",") if origin.strip()], allow_methods=["GET", "POST", "OPTIONS"], allow_headers=["*"])
app.include_router(render_router)
CASE_STORE = CaseStore(DIAGNOSTICS.storage)

PIPELINE_STAGE_DEFINITIONS = [
    (1, "file_upload_ingest", "File Upload / Ingest"), (2, "file_decode_normalization", "File Decode and Normalization"), (3, "provenance_integrity", "Provenance and Integrity"), (4, "channel_recording_assessment", "Channel and Recording Assessment"), (5, "speaker_identification_diarization", "Speaker Identification / Diarization"), (6, "speech_segmentation", "Speech Segmentation"), (7, "transcription_generation", "Transcription Generation"), (8, "transcript_alignment", "Transcript Alignment"), (9, "eligibility_reliability", "Eligibility and Reliability"), (10, "acoustic_feature_extraction", "Acoustic Feature Extraction"), (11, "prosodic_voice_quality", "Prosodic and Voice Quality Analysis"), (12, "temporal_pause_analysis", "Temporal and Pause Analysis"), (13, "linguistic_disfluency", "Linguistic and Disfluency Analysis"), (14, "question_answer_alignment", "Question / Answer Alignment"), (15, "within_speaker_baseline", "Within Speaker Baseline"), (16, "cross_method_evidence", "Cross Method Evidence Assembly"), (17, "evidence_convergence_conflict", "Evidence Convergence and Conflict"), (18, "candidate_classification", "Candidate Classification"), (19, "validation_calibration_gate", "Validation and Calibration Gate"), (20, "final_disposition", "Final Classification / Disposition"), (21, "audit_provenance_output", "Audit and Provenance Output")]
PIPELINE_FOUNDATION_STATUS = {"file_upload_ingest":"implemented","file_decode_normalization":"implemented","provenance_integrity":"implemented","channel_recording_assessment":"implemented","speaker_identification_diarization":"queued","speech_segmentation":"implemented_foundation","transcription_generation":"queued","transcript_alignment":"queued","eligibility_reliability":"implemented","acoustic_feature_extraction":"implemented","prosodic_voice_quality":"implemented_foundation","temporal_pause_analysis":"implemented_foundation","linguistic_disfluency":"conditional","question_answer_alignment":"conditional","within_speaker_baseline":"conditional","cross_method_evidence":"implemented_foundation","evidence_convergence_conflict":"implemented_foundation","candidate_classification":"implemented_guarded","validation_calibration_gate":"not_invoked","final_disposition":"implemented_guarded","audit_provenance_output":"implemented_foundation"}

def _new_stage_states():
...
def _set_stage(stage_states, stage_id, status, *, started_at=None, completed_at=None, duration_ms=None, outcome=None, error=None):
...
def _stage_build_summary():
    values=list(PIPELINE_FOUNDATION_STATUS.values()); return {"total":21,"implemented_foundations":sum(v.startswith("implemented") for v in values),"conditional_or_not_invoked":sum(v in {"conditional","not_invoked"} for v in values),"queued":sum(v=="queued" for v in values),"status_by_stage":PIPELINE_FOUNDATION_STATUS}

def _module_available(module_name: str) -> bool:
    try: return importlib.util.find_spec(module_name) is not None
    except (ImportError, ModuleNotFoundError, AttributeError, ValueError): return False

def _speech_runtime_status():
    transcription_provider=os.getenv("VOXVECTOR_TRANSCRIPTION_PROVIDER","").strip().lower() or "not_configured"; diarization_provider=os.getenv("VOXVECTOR_DIARIZATION_PROVIDER","").strip().lower() or "not_configured"; transcription_adapter=_module_available("faster_whisper"); diarization_adapter=_module_available("pyannote.audio"); hf_token=bool(os.getenv("HF_TOKEN") or os.getenv("HUGGINGFACE_TOKEN")); return {"transcription":{"configured_provider":transcription_provider,"adapter_installed":transcription_adapter,"execution_ready":transcription_provider!="not_configured" and transcription_adapter,"model":os.getenv("VOXVECTOR_WHISPER_MODEL","").strip() or None},"diarization":{"configured_provider":diarization_provider,"adapter_installed":diarization_adapter,"hf_token_configured":hf_token,"execution_ready":diarization_provider!="not_configured" and diarization_adapter and hf_token,"model":os.getenv("VOXVECTOR_DIARIZATION_MODEL","").strip() or None}}

def _file_sha256(path):
    with open(path,"rb") as handle: return hashlib.sha256(handle.read()).hexdigest()
ACOUSTIC_MODULE_PATH=os.path.abspath(_acoustic_module.__file__); PIPELINE_MODULE_PATH=os.path.abspath(sys.modules[VoxVectorPipeline.__module__].__file__); ACOUSTIC_SOURCE_SHA256=_file_sha256(ACOUSTIC_MODULE_PATH); PIPELINE_SOURCE_SHA256=_file_sha256(PIPELINE_MODULE_PATH); ACOUSTIC_RUNTIME_SIGNATURE=getattr(_acoustic_module,"RUNTIME_SIGNATURE","missing")
def _runtime_self_test():
    try: smoke_frames=np.zeros((2,1200),dtype=float); _acoustic_module.spectral_centroid(smoke_frames,24000); _acoustic_module.spectral_spread(smoke_frames,24000); return True,"passed"
    except Exception as exc: return False,f"{type(exc).__name__}: {exc}"

def _read_pcm_wav_extensible(data):
    ...
    if bytes_per_sample==1: audio=(np.frombuffer(payload,dtype=np.uint8).astype(np.float32)-128.0)/128.0
    elif bytes_per_sample==2: audio=np.frombuffer(payload,dtype="<i2").astype(np.float32)/32768.0
    elif bytes_per_sample==3:
        raw=np.frombuffer(payload,dtype=np.uint8).reshape(-1,3); values=raw[:,0].astype(np.int32)|(raw[:,1].astype(np.int32)<<8)|(raw[:,2].astype(np.int32)<<16); values=np.where(values&0x800000,values-0x1000000,values); audio=values.astype(np.float32)/8388608.0
    else: audio=np.frombuffer(payload,dtype="<i4").astype(np.float32)/2147483648.0
    if channels>1: audio=audio.reshape(-1,channels).mean(axis=1,dtype=np.float32)
    return audio,rate
...
def read_wav(data):
    try:
        ...
        if rate>MAX_SAMPLE_RATE: raise ValueError(f"Sample rate exceeds the {MAX_SAMPLE_RATE} Hz runtime limit")
        if width==1: audio=(np.frombuffer(frames,dtype=np.uint8).astype(np.float32)-128.0)/128.0
        elif width==2: audio=np.frombuffer(frames,dtype="<i2").astype(np.float32)/32768.0
        elif width==3:
            raw=np.frombuffer(frames,dtype=np.uint8).reshape(-1,3); values=raw[:,0].astype(np.int32)|(raw[:,1].astype(np.int32)<<8)|(raw[:,2].astype(np.int32)<<16); values=np.where(values&0x800000,values-0x1000000,values); audio=values.astype(np.float32)/8388608.0
        elif width==4: audio=np.frombuffer(frames,dtype="<i4").astype(np.float32)/2147483648.0
...
async def upload_case_source(case_id:str,request:Request,file:UploadFile=File(...),user:dict=Depends(require_developer)):
    ...
    if not filename.lower().endswith(".wav"): raise HTTPException(status_code=415,detail=f"Unsupported recording type: {content_type or 'unknown'}; expected WAV")
    data=await file.read()
    ...
    except StorageError as exc: raise HTTPException(status_code=503,detail=f"Media storage upload failed [request {rid}]") from exc
...
async def analyze_case_source(case_id:str,source_id:str,user:dict=Depends(require_developer)):
    ...
    try:
        ...
        speech_runtime=_speech_runtime_status(); transcription_provider=get_transcription_provider() if speech_runtime["transcription"]["execution_ready"] else None; diarization_enabled=os.getenv("VOXVECTOR_ENABLE_DIARIZATION_RUNS","").strip().lower() in {"1","true","yes","on"}; diarization_provider=get_diarization_provider() if diarization_enabled and speech_runtime["diarization"]["execution_ready"] else None
        acquisition_started=timer(); acquisition=await asyncio.to_thread(build_evidence_acquisition,audio,sample_rate,transcript_provider=transcription_provider,diarization_provider=diarization_provider); acquisition_duration=elapsed_ms(acquisition_started)
        acquisition_dict=acquisition.to_dict(); transcription_state=str(acquisition_dict.get("transcription_state") or "not_invoked"); diarization_state=str(acquisition_dict.get("diarization_state") or "not_invoked"); transcript=acquisition_dict.get("transcript"); multimodal_timeline=acquisition_dict.get("multimodal_timeline");
        transcript_tokens=[word.get("text") for word in (transcript.get("words") or []) if isinstance(word,dict) and word.get("text")] if isinstance(transcript,dict) else None
        pipeline_started=timer(); result=await asyncio.to_thread(VoxVectorPipeline().analyze,audio,sample_rate,transcript_tokens=transcript_tokens); completed_at=datetime.now(timezone.utc).isoformat(); pipeline_duration=elapsed_ms(pipeline_started)
        ...
        for stage_id,(status,outcome) in internal_completed.items(): _set_stage(stage_states,stage_id,status,completed_at=completed_at,outcome=outcome)
        _set_stage(stage_states,"linguistic_disfluency","complete" if transcript_tokens else "not_run",completed_at=completed_at,outcome="transcript-derived disfluency observations computed" if transcript_tokens else "transcript not attached")
        _set_stage(stage_states,"within_speaker_baseline","not_run",outcome="baseline not attached"); _set_stage(stage_states,"question_answer_alignment","not_run",outcome="question context not attached")
        _set_stage(stage_states,"speaker_identification_diarization","complete" if diarization_state=="completed" else ("failed" if diarization_state=="unavailable" else "not_run"),outcome="speaker turns acquired" if diarization_state=="completed" else ("diarization unavailable" if diarization_state=="unavailable" else "diarization not invoked in constrained runtime"))
        _set_stage(stage_states,"transcription_generation","complete" if transcription_state=="completed" else ("failed" if transcription_state=="unavailable" else "not_run"),outcome="timestamped transcript acquired" if transcription_state=="completed" else f"transcription {transcription_state}")
        _set_stage(stage_states,"transcript_alignment","complete" if multimodal_timeline else "not_run",completed_at=completed_at,outcome="timestamped transcript/audio timeline assembled" if multimodal_timeline else ("transcript unavailable" if transcript is None else "speaker alignment unavailable"))
        _set_stage(stage_states,"validation_calibration_gate","not_run",outcome="inferential validation gate not invoked")
        result_dict=VoxVectorPipeline.to_dict(result); final_run={"run_id":result.run_id,"analysis_id":result.run_id,"request_id":rid,"status":"completed","started_at":started_at,"completed_at":completed_at,"source_id":source_id,"pipeline_version":VoxVectorPipeline.software_version,"pipeline_duration_ms":pipeline_duration,"provider_acquisition_ms":acquisition_duration,"telemetry_scope":{"route_boundary_stages":["file_decode_normalization","provenance_integrity","channel_recording_assessment"],"composite_pipeline_internal_timing":"not independently instrumented"},"pipeline_build":{"total_stages":21,"completed":sum(s["status"] in {"complete","completed","success","succeeded"} for s in stage_states),"pending":sum(s["status"] in {"pending","running","processing","in_progress"} for s in stage_states),"not_run":sum(s["status"]=="not_run" for s in stage_states),"failed":sum(s["status"] in {"failed","error"} for s in stage_states)},"testing":{"current_commit_qa":"external_workflow_required","source_revision":SOURCE_REVISION,"historical_backend_baseline":{"passed":91,"duration_seconds":0.56}},"stages":stage_states,"result":result_dict,"acquisition":acquisition_dict,"transcript":transcript,"speakers":acquisition_dict.get("diarization",{}).get("speakers",[]) if isinstance(acquisition_dict.get("diarization"),dict) else [],"tracks":[]}
        envelope=compose_result_envelope(case=case,source=source,run=final_run,result=result_dict); final_run["result_envelope"]=envelope; updated_case=await asyncio.to_thread(CASE_STORE.update_run,str(user["id"]),case_id,final_run); return {"status":"ok","case":updated_case,"run":final_run,"result_envelope":envelope}
