...
class VoxVectorPipeline:
    ...
    def analyze(self, signal: np.ndarray, sample_rate: int, *, transcript_tokens: Sequence[str] | None = None, question_end_s: float | None = None, first_speech_s: float | None = None, first_substantive_s: float | None = None, baseline_values: Mapping[str, np.ndarray] | None = None) -> AnalysisResult:
        ...
        def add(feature: str, value: float | None, unit: str, provenance: dict) -> None:
            if value is None or not np.isfinite(value):
                return
            observations.append(Observation(method_id=str(provenance.get("method_id", "derived")), feature=feature, value=float(value), unit=unit, segment=(0.0, duration), quality=float(reliability.score), provenance=provenance))
        ...
        if signal.size and sample_rate > 0 and np.all(np.isfinite(signal)):
            ...
            if frames.size:
                ...
                add("spectral_flux", summarize(flux_values)["mean"], "ratio", {"method_id": "spectral.flux", "summary": summarize(flux_values)})
                add("spectral_rolloff", summarize(rolloff_values)["mean"], "Hz", {"method_id": "spectral.rolloff", "summary": summarize(rolloff_values)})
                ...
                for index in range(formant_matrix.shape[1]):
                    ...
                    add(f"F{index + 1}_candidate", summarize(values)["mean"], "Hz", {"method_id": "formants.frame_tracking", "summary": summarize(values), "formant_index": index + 1})
        ...
        limitations = ["No deception inference is produced by this observational pipeline.", "Evidence records are neutral observational groupings and do not indicate deception.", "All research-derived features require task-specific reliability and scientific validation before inferential use."]
