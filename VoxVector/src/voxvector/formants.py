...
def estimate_formants(frame: np.ndarray, sample_rate: int, n_formants: int = 4, min_hz: float = 200.0, max_hz: float = 5000.0) -> np.ndarray:
    ...
    if not(0<min_hz<effective_max_hz): raise ValueError("invalid formant frequency range")
    centered=frame-np.mean(frame); spectrum=np.abs(np.fft.rfft(centered*np.hanning(frame.size))); frequencies=np.fft.rfftfreq(frame.size,1.0/sample_rate); valid=(frequencies>=min_hz)&(frequencies<=effective_max_hz); indices=np.flatnonzero(valid)
