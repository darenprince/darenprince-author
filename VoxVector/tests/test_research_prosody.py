...
def test_contour_summary_ignores_nonfinite_observations():
    result=contour_summary(np.array([100.,np.nan,120.]),np.array([0.,1.,2.])); assert result["mean"]==110.; assert result["range"]==20.; assert np.isclose(result["slope"],10.0, rtol=0.0, atol=1e-12)
