...
from voxvector.evidence_acquisition import DiarizationResult, TranscriptResult, _speech_from_frames, build_evidence_acquisition
...
class StubProvider:
    ...
    def transcribe(self, signal, sample_rate):
        return TranscriptResult(provider_id=self.provider_id, language="en", text="test transcript", segments=(), words=())
...
class StubDiarizationProvider:
    ...
    def diarize(self, signal, sample_rate):
        return DiarizationResult(provider_id=self.provider_id, speakers=("SPEAKER_00",), segments=())
...
def test_build_evidence_acquisition_profiles_audio_and_timeline(monkeypatch):
    ...
    signal = np.concatenate([np.zeros(800), np.ones(2400) * 0.1, np.zeros(800)]).astype(np.float32)
...
def test_speech_detection_uses_bounded_frame_groups():
    sample_rate = 8000
    signal = np.zeros(sample_rate * 12, dtype=np.float32)
    signal[2 * sample_rate : 10 * sample_rate] = 0.1
    segments = _speech_from_frames(signal, sample_rate, frame_size=200, hop_size=80, chunk_frames=32)
    assert segments
...
def test_transcription_provider_contract_is_optional_and_explicit():
    result = build_evidence_acquisition(np.ones(1600, dtype=np.float32) * 0.1, 8000, transcript_provider=StubProvider())
...
def test_explicit_diarization_provider_is_recorded():
    result = build_evidence_acquisition(np.ones(1600, dtype=np.float32) * 0.1, 8000, diarization_provider=StubDiarizationProvider())
...
def test_environment_provider_selection_is_used(monkeypatch):
    ...
    monkeypatch.setattr(providers, "get_transcription_provider", lambda: StubProvider())
    result = build_evidence_acquisition(np.ones(1600, dtype=np.float32) * 0.1, 8000)
    assert result.transcription_state == "completed"
    assert result.transcript.text == "test transcript"


def test_provider_failures_are_explicit_and_non_fatal():
    result = build_evidence_acquisition(np.ones(1600, dtype=np.float32) * 0.1, 8000, transcript_provider=FailingProvider(), diarization_provider=FailingProvider())
